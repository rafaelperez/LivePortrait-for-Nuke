...
class LivePortraitNukeFaceDetection(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x - torch.tensor([104.0, 117.0, 123.0], device=device).reshape(1, 3, 1, 1)
...
class LivePortraitNukeMotionExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        rot_mat = self.get_rotation_matrix(pitch, yaw, roll)  # Bx3x3
    ...
    def get_rotation_matrix(self, pitch_, yaw_, roll_):
...
def trace_face_detection_nuke(run_test=False):
    sf3d_face_detection = torch.jit.load("./pretrained_weights/sfd_detector_traced.pt").eval().cuda()
...
if __name__ == "__main__":
    ...
    run_test = False
    run_test = True
    trace_face_detection_nuke(run_test=False)
